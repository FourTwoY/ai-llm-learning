...
from services.embedding_service import build_chunk_embeddings, save_embeddings, load_embeddings
...
def get_runtime_settings() -> tuple[dict, dict]:
    cfg = get_config()
    return cfg["paths"], cfg["retrieval"]


def ensure_embeddings_ready():
    paths, _ = get_runtime_settings()
    embeddings_path = Path(paths["embeddings_file"])
    if embeddings_path.exists():
        return

    chunks = chunk_documents(paths["chunks_file"])
    items, meta = build_chunk_embeddings(chunks)
    save_embeddings(paths["embeddings_file"], items, meta)
...
def ask(request: AskRequest):
    try:
        ...
        paths, retrieval_cfg = get_runtime_settings()
        top_k = request.top_k or retrieval_cfg["top_k"]
        rerank_top_n = retrieval_cfg["rerank_top_n"]

        ensure_embeddings_ready()
        embedded_chunks = load_embeddings(paths["embeddings_file"])

        if request.use_rerank:
            retrieved_chunks = retrieve_chunks(question, embedded_chunks, top_k=top_k)
...
def search(request: SearchRequest):
    try:
        ...
        embedding_results = retrieve_chunks(question, embedded_chunks, top_k=top_k)

        if request.use_rerank:
            rerank_results = rerank_chunks(
                question,
                embedding_results,
                top_k=min(rerank_top_n, len(embedding_results)),
            )
        else:
            rerank_results = []
